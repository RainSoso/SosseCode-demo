<template>
  ...
  <div v-if="options.type === 'input'">
    <div v-if="options.propName === 'defaultChoose'">
      <el-input
        v-model="stylePropsObj[options.propName]"
        disabled
        style="width: 180px"
      />
    </div>
  ...
  <div v-else-if="options.type === 'select'">
    <el-select v-model="stylePropsObj[options.propName]" style="width: 180px">
      <el-option
  ...
  <div v-else-if="options.type === 'colorPicker'">
    <el-color-picker v-model="stylePropsObj[options.propName]" />
  </div>
  ...
  <div v-else-if="options.type === 'switch'">
    <div v-if="options.propName === 'flex'">
      <el-switch v-model="stylePropsObj[options.propName]" disabled />
    </div>
    <div v-else>
      <el-switch v-model="stylePropsObj[options.propName]" />
    </div>
  </div>
  ...
  <div v-else-if="options.type === 'select-picker'">
    <el-select
      v-model="stylePropsObj[options.propName]"
      :multiple="stylePropsObj['multiple']"
      :clearable="stylePropsObj['clearable']"
      :collapse-tags="stylePropsObj['collapse-tags']"
      :collapse-tags-tooltip="stylePropsObj['collapse-tags-tooltip']"
      :max-collapse-tags="stylePropsObj['max-collapse-tags']"
      style="width: 180px"
    >
      <el-option
        v-for="item in stylePropsObj.selectList"
        :key="item.value"
        :label="item.label"
        :value="item.value"
  ...
  <div v-else-if="options.type === 'checkList'">
    ...
    <el-checkbox-group v-model="stylePropsObj.defaultChoose">
      <el-checkbox
        v-for="(check, index) in stylePropsObj[options.propName]"
        :label="check.label"
        :value="check.value"
        :key="check.value"
      ...
      >
        ...
        <el-icon
          ...
          @click="
            handelCheckDelete(
              stylePropsObj[options.propName],
              stylePropsObj.defaultChoose,
              index
            )
          "
          ><Delete
    ...
    <div
      class="addCol"
      @click="handleAddDetail(stylePropsObj[options.propName])"
    >
      添加列 +
    </div>
  </div>
  ...
  <div v-else-if="options.type === 'radioList'">
    ...
    <el-radio-group v-model="stylePropsObj.defaultChoose">
      <el-radio
        v-for="(check, index) in stylePropsObj[options.propName]"
        :label="check.label"
        :value="check.value"
  ...
  <div v-else-if="options.type === 'date-picker'">
    <el-date-picker
      ...
      style="width: 180px"
      format="YYYY-MM-DD"
      :type="stylePropsObj['type']"
      :unlink-panels="stylePropsObj['unlink-panels']"
  ...
  <div v-else-if="options.type === 'rate'">
    <el-rate
      v-model="stylePropsObj[options.propName]"
      :max="stylePropsObj['max']"
      :allow-half="stylePropsObj['allow-half']"
      :disabled="stylePropsObj['disabled']"
      :clearable="stylePropsObj['clearable']"
      style="width: 180px"
    />
  </div>
  ...
  <div v-else-if="options.type === 'radio-button'">
    <el-radio-group v-model="stylePropsObj[options.propName]">
      <el-radio-button
        v-for="item in options.options"
        :key="item.value"
        :label="item.label"
  ...
  <div v-else-if="options.type === 'vertical'">
    <el-switch v-model="stylePropsObj[options.propName]"></el-switch>
  </div>
  ...
  <div v-else-if="options.type === 'colData'">
    <el-card style="width: 240px">
      <div class="tableColBox">
        <el-table
          :data="stylePropsObj[options.propName]"
  ...
  <div v-else-if="options.type === 'height-switch'">
    <el-switch v-model="stylePropsObj[options.propName]" />
    <div v-if="stylePropsObj[options.propName]">
      高度：
      <el-input-number v-model="stylePropsObj[options.propName]" />
      px
    </div>
  </div>
...
const handleDelete = (item, index, row) => {
  if (item === "colData") {
    stylePropsObj.value.colData.splice(index, 1);
  }
  if (item === "desColData") {
    stylePropsObj.value.desColData.splice(index, 1);
  }
};
const handleSelectDelete = (item1, item2, index) => {
  const deleteItemValue = item1[index].value;
  item1.splice(index, 1);
  stylePropsObj.value.content = "";
};
const handelCheckDelete = (item1, item2, index) => {
  const deleteItemValue = item1[index].value;
  item1.splice(index, 1);
  if (Array.isArray(item2)) {
    stylePropsObj.value.defaultChoose = item2.filter(
      (item) => item !== deleteItemValue
    );
  }
  if (typeof item2 === "string") {
    stylePropsObj.value.defaultChoose = "";
  }
};
...
const setLabelSuffixForm = () => {
  if (isFormTrue.value === true) {
    stylePropsObj.value.labelSuffixText = "：";
  } else {
    stylePropsObj.value.labelSuffixText = "";
  }
};
